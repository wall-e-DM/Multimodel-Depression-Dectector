...
def exp_clip_text_multi_modal(groud: list):
    ...
    for item in groud_list:
        ...
        clip_output = get_clip_output(image_url)
...
def exp_clip_only(ground:List):
    ground_labels = []
    pred_labels = []
    for item in groud_list:
        ground_label, image_url = item[0], item[1]
        clip_output = get_clip_output(image_url)
        # use argmax to get the pred_label
        pred_label = np.argmax(clip_output)
        ground_labels.append(ground_label)
        pred_labels.append(pred_label)
        index_mapper ={
            0: "not depression",
