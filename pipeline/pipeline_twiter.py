...
############################  import necessities ###############################
from utils import get_ensemble_ouput, groundtruth_generator, modality_fusion, twiter_loader, binarize, drawROC
from image2text.api import get_clip_output
...
def exp_clip_text_multi_modal(groud: list):
    ...
    drawROC(ground_labels, pred_scores, 'Twitter_multi_modal')
    ...
    plt.savefig(f"./plots/confusion_matrix-mutlti-{timestamp}.png")

    with open("./log_twiter.txt", "a") as f:
        f.write("-----------------------------\n")
...
def exp_ensemble_model(groud:List):
    ...
    drawROC(ground_labels, pred_scores, 'Twitter_ensemble')
    ...
    plt.ylabel('Actual label')
    plt.xlabel('Predicted label')
    plt.savefig(f"./plots/confusion_matrix-ensemble-{timestamp}.png")
